...
export function mapDecoderResultToTransaction(dr: any): Transaction {
  ...
  return {
    ...
    blockNumber: String(dr.blockNumber ?? '0').replace(/n$/, ''),
    ...
    gasUsed: String(dr.gasUsed ?? '0').replace(/n$/, ''),
    gasPrice: String(dr.gasPrice ?? '0').replace(/n$/, ''),
    to: dr.to ?? '',
    from: dr.from ?? '',
    value: String(dr.value ?? '0').replace(/n$/, ''),
    input: dr.input ?? '',
